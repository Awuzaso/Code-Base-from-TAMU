...
int main(int argc, char *argv[])
{
  ...
  if(transform == "rotate"){
	...
	float pX, pY;
	cout << "Perspective x?->" << endl;
	cin >> pX;
	cout << "Perspective y?->" << endl;
	cin >> pY;
	perspective(pX,pY);
  }
  else if(transform == "bilinear"){
	float bX, bY;
	cout << "Value x?->" << endl;
	cin >> bX;
	cout << "Value y?->" << endl;
  ...
  for(y = height-1; y >= 0; y--){
	...
	/*Pixel value is placed within pixmap.*/
      ppmPixmap[pixel].blue = fgetc(input_fileRead);
        //cout << "Done." << endl;
      //cout << ppmPixmap[pixel].red << " " << ppmPixmap[pixel].green << " " <<  ppmPixmap[pixel].blue << endl;
    }
  }	
}
//////////////////////////////////////////////////////////////////
...
void writePPM(){
	...
	height and introduce linebreak.*/ 
       fprintf(output_fileRead, "%d", maxcolor);
